...
bool glutGUI::orthographic = false;
bool glutGUI::oblique = false;
...
void glutGUI::defaultKey(unsigned char key, int x, int y)
{
    ...
    case 'q':
        ...
        case 'G':
           ...
           if (cam->estilo == 1) {
               delete cam;
               if (posCam%4==0) cam = new CameraDistante();
               if (posCam%4==1) cam = new CameraDistante(5,5,5, 0,0,0, 0,1,0); // ISOMETRIC 3 eixos iguais
               if (posCam%4==2) cam = new CameraDistante(5,2,5, 0,0,0, 0,1,0); // DIMETRIC 2 eixos iguais
               if (posCam%4==3) cam = new CameraDistante(1,6,8, 0,0,0, 0,1,0); // TRIMETRIC
               if (posCam%4==4) cam = new CameraDistante(savedCamera[0],savedCamera[1],savedCamera[2],savedCamera[3],savedCamera[4],savedCamera[5],savedCamera[6],savedCamera[7],savedCamera[8]);
       ...
       case 'T':
            static int posCam1 = 0;
            posCam1++;
            if (cam->estilo == 1) {
                delete cam;
                if (posCam1%4==0) cam = new CameraDistante(); //CameraDistante(0,1,5, 0,1,0, 0,1,0);
                if (posCam1%4==1) cam = new CameraDistante(0,1,5, 0,1,0, 0,1,0);
                if (posCam1%4==2) cam = new CameraDistante(5,1,0, 0,1,0, 0,1,0);
                if (posCam1%4==3) cam = new CameraDistante(0,5,0, 0,1,0, 0,0,-1); // NESSE CASO up é -1 pq ta olhando z negativo. // CameraDistante(1,1,0, 1,0,0, 0,0,-1); // ISOMETRIC
                // como funciona o gluLookat prova

             // GUARDA AS POSIÇÕES DA CÂMERA
                if (posCam1%4==4) cam = new CameraDistante(savedCamera[0],savedCamera[1],savedCamera[2],savedCamera[3],savedCamera[4],savedCamera[5],savedCamera[6],savedCamera[7],savedCamera[8]);
                 } else if (cam->estilo == 2) {
                     delete cam;
                     cam = new CameraDistante();
                }
       ...
       //PARA A PROJEÇÃO PERSPECTIVA COM PONTOS DE FUGA
       case 'V':
           static int posCam2 = 0;
           posCam2++;
           if (cam->estilo == 1) {
               delete cam;
               if (posCam2%4==0) cam = new CameraDistante(); //CameraDistante(0,1,5, 0,1,0, 0,1,0);
//               if (posCam2%4==1) cam = new CameraDistante(5,1,0, 0,1,0, 0,1,0); // 1 PONTO DE FUGA
//               if (posCam2%4==2) cam = new CameraDistante(-5,0,-5, 0,1,0, 0,1,0); // 2 PONTOS DE FUGA
//               if (posCam2%4==3) cam = new CameraDistante(8,8,-8, 0,0,0, 0,1,0); // 3 PONTOS DE FUGA
               if (posCam2%4==1) cam = new CameraDistante(0.3,0.4,-1.2, 0.3,0.4,0, 0,1,0); // 1 PONTO DE FUGA
               if (posCam2%4==2) cam = new CameraDistante(1.5,0.6,-1.2, 0.3,0.4,0, 0,1,0); // 2 PONTOS DE FUGA
               if (posCam2%4==3) cam = new CameraDistante(0.8,0.8,-0.8, 0,0,0, 0,1,0); // 3 PONTOS DE FUGA

               // GUARDA AS POSIÇÕES DA CÂMERA
               if (posCam2%4==4) cam = new CameraDistante(savedCamera[0],savedCamera[1],savedCamera[2],savedCamera[3],savedCamera[4],savedCamera[5],savedCamera[6],savedCamera[7],savedCamera[8]);
           } else if (cam->estilo == 2) {
               delete cam;
               cam = new CameraDistante();
           }
    ...
    case 'g':
        glutGUI::orthographic = !glutGUI::orthographic;
        break;

    case '*':
        glutGUI::oblique = !glutGUI::oblique;
        break;
